...
namespace SimpleUploadExcelHelper
{
    public class NPOIExcelHelper:IDisposable
    {
        ...
        /// <returns></returns>
        public DataTable ExcelToDataTable(string sheetName, int columnNameRow)
        {
            ISheet sheet = null;
            DataTable data = new DataTable();
            ...
            try
            {
                ...
                if (sheet != null)
                {
                    if (columnNameRow >= 0)
                    {
                        IRow firstRow = sheet.GetRow(columnNameRow);
                        int cellCount = firstRow.LastCellNum; //一行最后一个cell的编号 即总的列数
                        ...
                        data.Columns.Add("原始行号");
                        data.Columns.Add("错误信息");
                        startRow = sheet.FirstRowNum + columnNameRow + 1;
