...
namespace BatchUploadExcelHelper
{
    ...
    {
        public static readonly string SaveErrorFileRootConfig = "SaveErrorFileRoot";
        ...
        public  static ExcelFileInfo GetFile<T>(List<T> errors) where T : Entities.EntityBase
        {
            ...
            var saveErroRoot = ConfigurationManager.AppSettings["SaveErrorFileRoot"];
            if (saveErroRoot == null)
            {
                throw new Exception("没有配置导入错误文件保存路径");
            }

            var filePath =  Path.Combine(saveErroRoot.ToString(), "ImportErrorFiles", importType.Name);
